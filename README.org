...
- [[#a-key-the-mode-gets-to-keep][A key the mode gets to keep]] — =p=, where the mode has one
...
* A Key the Mode Gets to Keep
:PROPERTIES:
:CUSTOM_ID: a-key-the-mode-gets-to-keep
:END:

=p= is DONKEY's paste — except in a buffer whose major mode has a =p=
of its own, where it is the mode's.

| Where | =p= runs |
|-------+----------|
| a file you are editing | =donkey-yank=, DONKEY's paste |
| a compilation log | =previous-error-no-select= |
| a help buffer | =help-goto-previous-page= |
| a man page | =Man-previous-section= |
| a PDF in =doc-view= | =doc-view-previous-page= |

The reason is that =p= is the one key the rest of Emacs agrees about.
Ninety-three of the major modes Emacs ships bind it, and ninety of
those mean the previous line, the previous page, the previous error or
the previous item. DONKEY answered all of them with paste, which in a
buffer you cannot type into could only ever have failed.

=C-h k= tells you which you have: the key really is the mode's command
in that buffer, not DONKEY's command pretending.

Nothing else changes. =j=, =k=, =d= and the rest are DONKEY's
everywhere, and =p= is only given away where the mode has a command for
it — so in a file you are editing nothing is different at all, most
modes binding no plain letters whatever.

Three kinds of binding are passed over rather than taken: a command
that would type, because NORMAL state does not type; =describe-mode=,
which is =C-h m= and which many modes put on a key; and the stubs a
mode uses to say its buffer cannot be edited.

** Turning it off, or widening it

#+begin_src emacs-lisp

  ;; p is DONKEY's paste everywhere, as before
  (setopt donkey-handed-back-keys nil)

  ;; or hand back more than one key
  (setopt donkey-handed-back-keys '(?p ?u))

#+end_src

Characters, not strings. A key you add is given away on the same terms:
only where the mode binds it, never where the mode would type with it.
...
#+begin_src emacs-lisp
  ...
  (use-package donkey
    ...
    ;; Keys the major mode keeps where it binds one of its own.  p is
    ;; the previous line, page, error or item in 93 of the modes Emacs
    ;; ships; nil gives DONKEY its paste back everywhere.
    (setopt donkey-handed-back-keys '(?p))                       ; default
...
| =donkey-handed-back-keys=          | =(?p)=                           | keys the major mode keeps where it binds one |
