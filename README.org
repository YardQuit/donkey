...
3. =RET= does nothing in a buffer you are editing, and is handed back
   to Dired, Magit and the like in buffers you are not.
...
mode: Dired keeps =+=, =(= and =<=, Info keeps =[= and =]=, a help
buffer keeps =<=. A wrap key is borrowed only for as long as a
...
=dired-find-file= in Dired, =magit-visit-thing= in Magit,
=Info-follow-nearest-node= in Info. In Org and Markdown it follows the
link at point, toggles the checkbox, or flips the TODO — see [[#enter-rules][Enter
Rules]] to change or extend that.
...
#+begin_src emacs-lisp
  ...
  (with-eval-after-load 'donkey
    (dolist (mode '(dired-mode magit-mode ibuffer-mode pdf-view-mode))
...
want, since a wdired buffer is one you are editing.
...
reading stopped: a REPL or a terminal that is not a comint derivative
and is not named here gets NORMAL state, and belongs on the list the
day somebody meets it. Add it as above, and please open an issue so
...
own on =$=, =\= and =^=. Those are refused by name:
...
#+begin_src emacs-lisp
  ...
  (use-package donkey
    ...
    :config
    ;; modes that are really applications
    (dolist (mode '(dired-mode ibuffer-mode magit-mode pdf-view-mode))
      (add-to-list 'donkey-excluded-modes mode))
...
|------------------------------------+----------------------------------+--------------|
| =donkey-excluded-modes=            | 10 modes, see above              | where DONKEY steps aside completely |
...
| =donkey-self-insert-commands=      | 16 commands                      | mode commands that type, refused in NORMAL state |
