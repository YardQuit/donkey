...
- [[#pairing-as-you-type][Pairing as you type]] — closing a delimiter for you
- [[#the-enter-key][The Enter key]]
- [[#search-and-replace][Search and replace]]
- [[#editing-every-match-at-once-f][Editing every match at once: =f=]]
...
- [[#pairing-every-setting][Pairing: every setting]] — per mode, per delimiter, and who does it
...
Sixteen lessons, ending with the five that go beyond a single cursor
— wrapping a selection, the characters the keyboard does not have,
editing every match at once and choosing what it holds, and cursors in
a column:
...
14. Editing every match at once (Split mode)
15. Choosing what a split holds (selections, banks, regexps)
16. Cursors in a column
...
A rectangle costs a motion the same however tall it is: only the rows
a window could show are highlighted, and a middle-click in another
program is answered with the rectangle when it asks, rather than
Emacs copying every row after every key. What the middle-click pastes
is unchanged: the rectangle as it stands, as it was left, or what a
change took out of it.
...
- an opening delimiter writes its closing half and leaves the cursor
  between the two;
- the closing half, typed where that character already stands, steps
  over it rather than doubling it;
- =DEL= between the halves of an empty pair takes both, and =DEL=
  anywhere else is whatever your major mode bound it to;
- a count types and pairs nothing: =C-u 3 (= gives you three of them.
...
under a count rather than not at all. [[#pairing-every-setting][Pairing: every setting]] says
who does the job when, and what each one does differently.

** Which delimiters pair
...
A list replaces the set rather than adding to it, so name every
character you want, not only the extra one.
...
| ~=~ =*= =~= \vert =\= =/= =:= =+= =_= =$= | no     | ordinary punctuation       |
...
** If you already use a pairing package

You do not have to choose, and you do not have to configure
anything. DONKEY works out who should do the job.
...
Where Emacs is doing it, DONKEY hands Emacs the delimiters, so a
pair you added to the table is one Emacs pairs too. Where
Smartparens is on, DONKEY stays out of the way entirely.

To see who has the job in the buffer you are in, run =M-x
donkey-debug-platform= and read its pairing line.

That is the whole of everyday use. [[#pairing-every-setting][Pairing: every setting]] has the
...
:PROPERTIES:
:CUSTOM_ID: editing-every-match-at-once-f
:END:
...
While you type, what you write appears at every match at once.
...
buffer is a selection like any other, so you reach it by choosing it.
...
- =\s-= is whitespace; =\s+= here matches nothing at all.
- =\S-= is anything but whitespace; =\S+= here matches every character.
- =[0-9]= is a digit; =\d= here is the letter =d=.
- =\(a\|b\)= is =a= or =b=; plain parentheses match themselves.
- =x\{3\}= is =xxx=; plain braces match themselves.
...
Where the matches differ, the split types only at their edges: moving
into one ends it, since there is no one text to edit in all of them.
Editing inside matches that differ is what a multiple-cursors package
is for; this is not one.

** Cursors in a column: =T=
...
instead, and =DEL= takes back the cursor added last. A line shorter than the column gets its cursor at
its end.
...
The real cursor stays on point's line. The other cursors are drawn in the real cursor's shape,
a box or a bar as the state changes, and always in the text's own
color rather than the real cursor's; a background given to the face
=donkey-split-cursor-face= is used instead. Every cursor then does
...
cursor went. A key =m i= would ask for is asked once.
...
an edit that fails at one cursor is taken back at all of them. =z z=
and =C-l= move the view and keep the cursors.

Every key runs once at every cursor, so cursors are for a screenful of
lines rather than a whole book: at a thousand a key takes a few
milliseconds, at tens of thousands a second or more. =T= refuses to
make more than =donkey-split-cursor-limit= (1000), and says so; set it
higher, or to =nil= for no limit, or set
=donkey-split-cursor-limit-ask= to be asked instead. =c= on a rectangle
taller than the limit asks for its text in the minibuffer and writes it
on every row at once. =f= has no limit: its places cost far less.
...
* Pairing: every setting
:PROPERTIES:
:CUSTOM_ID: pairing-every-setting
:END:

Everyday use is [[#pairing-as-you-type][Pairing as you type]]: turn the mode on, and
choose the delimiters. This is everything else it can be told.
...
** What =safe= holds back, and putting one back

Every =no= in the =safe= table under [[#pairing-as-you-type][Pairing as you type]] is a setting
...
#+begin_src emacs-lisp
  ...
  (use-package donkey
    ...
    (setopt donkey-split-cursor-limit 1000)                      ; default
...
| =donkey-split-cursor-limit=        | =1000=                           | most cursors =T= makes; =nil= for no limit |
