...
Everything above, in one block, as a starting point to cut down:
...
#+begin_src emacs-lisp
  ...
  (use-package donkey
    ...
    :config
    ;; more modes that are really applications (16 ship on the list)
    (dolist (mode '(pdf-view-mode eww-mode elfeed-search-mode))
      (add-to-list 'donkey-excluded-modes mode))

    ;; cursor
    (setq donkey-cursor-normal 'box
          donkey-cursor-insert '(bar . 2))

    ;; a longer jump-back ring
    (setq donkey-position-ring-max 20)

    ;; a pair of my own, claimed now rather than at first idle
    (setopt donkey-mark-pair-delimiters
            (cons '(?# . ?#) donkey-mark-pair-delimiters))

    ;; input methods under SPC i
    (setq donkey-input-methods
          '(("s" "swedish" "swedish-postfix")
            ("p" "polish"  "polish-slash")))
  ...
  ;; one key back to its mode, outside the block because it is that
  ;; mode's business rather than DONKEY's
  (with-eval-after-load 'org-noter
    (define-key org-noter-doc-mode-map
                [remap donkey-insert-here] #'org-noter-insert-note))
