...
want, since a wdired buffer is one you are editing. =C-x C-q= puts you
there in INSERT state, ready to type a new name; =C-g= gives you
NORMAL state and DONKEY's editing keys.
...
#+begin_src emacs-lisp
  ...
  ;; add one you have met
  (with-eval-after-load 'donkey
    (add-to-list 'donkey-self-insert-commands 'my-mode-electric-quote)
...
met yet will type. If you find one, add it and open an issue.
...
Take the parent off and name the children you still want instead:
