...
The name reaches both readers of the keymap. which-key shows it in its
popup, and =?= gives it a row of its own above the keys it holds:

#+begin_example

  SPC o          +open/apps
  SPC o g        magit-status  magit
  SPC o m        ement-connect  ement

#+end_example

The plus is which-key's mark for a prefix, and the row has no command
button because a prefix runs nothing. A prefix you did not name gets
no row — there is nothing to say about it — and its keys are still
listed.
...
*** The two places are not the same place

A name in the binding reaches both readers. A name in which-key's
table reaches which-key alone, and wins there if you use both:

| Named in          | which-key shows | =?= shows     |
|-------------------+-----------------+---------------|
| the binding       | your name       | your name     |
| which-key's table | your name       | nothing       |
| both              | which-key's     | the binding's |
| neither           | =+prefix=       | nothing       |

So name a prefix of your own where you bind it. =?= reads the keymap
and nothing else, so a name that lives in which-key's table is
invisible there — the prefix's keys are still listed, and the prefix
simply has no row. Put a different name in each place and the two
displays disagree with no warning that they do.

That is the whole reason to reach for the table: a prefix belonging to
another package, which you did not bind and so cannot name in place.
