...
doing the pairing — see below — =DEL= and counts are Emacs's, and it
pairs once under a count rather than not at all.
...
Every =no= above is a setting rather than a rule. The thirteen
characters those rows name are =donkey-pair-safe-exclusions=, and you
