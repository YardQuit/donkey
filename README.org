...
selected: select what the pair holds and the pair comes off; select
the pair as well and another goes on around it.
...
#+begin_src emacs-lisp
  (setopt undo-limit (* 100 1024 1024)          ; history kept, per buffer
          ...
          undo-outer-limit (* 200 1024 1024))   ; the largest single step recorded
#+end_src
