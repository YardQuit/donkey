...
- [[#keys-the-mode-gets-to-keep][Keys the mode gets to keep]] — twelve, where the mode has one
...
Elsewhere, in the buffers a program made for you, twelve letters go to
the major mode when the mode has a command of its own for them:

#+begin_example

  p  u  o  c  U  P  C  O        a  i  A  I

#+end_example

They are the keys whose DONKEY command could not have worked in such a
buffer anyway. Eight of them only modify — =p= and =P= paste, =u=
undoes, =U= redoes, =c= changes, =C= comments, =o= and =O= open a line
— and a buffer you cannot type into refuses every one. The other four
only turn INSERT state on: =a=, =i=, =A= and =I= used to leave you in a
...
The delete family is deliberately absent for that reason. =d=, =x= and
=D= are the keys where a mode's own command removes a buffer, a
process or a file with no second step — =process-menu-delete-process=,
=tab-switcher-delete=, =tar-expunge= — and DONKEY's binding is what
stands in the way. Every motion and selection key is absent too: =h=,
...
#+begin_src emacs-lisp
  ...
  (use-package donkey
    ...
    ;; twelve whose DONKEY command could not have worked there anyway.
...
| =donkey-handed-back-keys=          | 12 keys, see above               | keys the major mode keeps where it binds one |
