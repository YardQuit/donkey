...
somewhere to go. Thirty-three sections ship; twenty-three of them give
=h= or =l= a meaning:
...
Eleven sections name the =prose= package instead of pairs: =Man-mode=,
=woman-mode=, =help-mode=, =apropos-mode=, =shortdoc-mode=,
=dictionary-mode=, =messages-buffer-mode=, =debugger-mode=,
=vc-annotate-mode=, =log-view-mode= and DONKEY's own bindings chart.
...
#+begin_src emacs-lisp
  ...
  (use-package donkey
    ...
    ;; Twelve modes ship on the list: the terminals and REPLs, where
    ...
    ;; and l mean there; j and k are never written down.  Seventeen
...
|------------------------------------+----------------------------------+--------------|
| =donkey-excluded-modes=            | 12 modes, see above              | where DONKEY steps aside completely |
| =donkey-support-modes=             | 17 sections, see above           | what =h= and =l= do in a support mode |
