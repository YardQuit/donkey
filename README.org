...
fail to mention the mode you are standing in. =C-h v major-mode= tells
you where you are, and =M-x describe-mode= names the parent it derives
from; the entry that matched is the first one on the list that the
parent chain reaches.
