...
#+begin_src emacs-lisp
  ...
  (use-package donkey
    ...
    ;; fifteen that mean the same kind of thing on both sides.
    ;; nil gives every letter back to DONKEY everywhere.
    (setopt donkey-handed-back-keys '(?p ?u ?o ?c ?U ?P ?C ?O
                                      ?a ?i ?A ?I
                                      ?d ?x ?D))                 ; default

    ;; What h and l do where a mode has somewhere to go.  j and k need
    ;; no entry: the modes that matter remap next-line already.
    (setopt donkey-mode-navigation
