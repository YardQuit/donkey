...
- [[#split-mode][Editing every match at once: =f=]]
...
Fourteen lessons, ending with the three that go beyond a single
cursor — wrapping a selection, the characters the keyboard does not
have, and editing every match at once:
...
What =c= and =d= remove reaches the kill ring once, not once per match,
so =p= pastes what was there rather than a column of copies.
...
line and no further, so =^= and =$= stay on the lines you banked. A =v= selection made after banking is searched with the
bank, exactly as selected; the line the cursor happens to be on is not
added. Opening the split spends the bank, as =y= and =d= do, so =C-g=
on the split ends it without bringing the bank back; if nothing
matches, the bank stays. A live =m v= block is searched instead of the
bank and leaves it alone.
...
Every match gets the same text. A regexp whose matches differ — =^[a-z]+=
over =alpha=, =beta=, =gamma= — is refused rather than replacing them
all with the first. Editing matches independently is what a
multiple-cursors package is for; this is not one.
