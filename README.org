...
Dired, Info and Magit are not in the table, and an entry for them would
do nothing: DONKEY steps aside in those, so it has no key to give away.
If you take one off [[#modes-where-donkey-steps-aside][the excluded list]], these are the entries to add:

#+begin_src emacs-lisp

  (with-eval-after-load 'donkey
    (setopt donkey-mode-navigation
            (append '((dired-mode (?h . dired-up-directory)
                                  (?l . dired-find-file))
                      (Info-mode  (?h . Info-up)
                                  (?l . Info-follow-nearest-node))
                      (magit-mode (?h . magit-dispatch)
                                  (?l . magit-log)))
                    donkey-mode-navigation)))
