...
Two of the seventeen are there only because the rule misses them:
=Custom-mode= and =org-agenda-mode= are neither derived from
=special-mode= nor read-only, and a section makes a mode a support mode
whatever the rule thinks.
