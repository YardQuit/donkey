...
last of them. Once the split has ended, one =u= takes back everything
it wrote, at every match, and =U= puts it back.
...
than the column gets its cursor at its end. When a motion leaves the
cursors in different columns, ~=~ lines them up under the real one
again; with selections it wraps them, as any delimiter does.
...
The real cursor stays on point's line. The other cursors are drawn in
