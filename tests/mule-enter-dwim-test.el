...
(ert-deftest mule-enter-dwim-non-editing-enter-handler-valid-ret-binding ()
  ...
  (let ((expected-cmd #'dired-find-file))
    (mule-enter-dwim-test--with-local-ret expected-cmd 'dired-mode
                                          (let ((result (mule--non-editing-enter-handler)))
                                            (should (eq result expected-cmd))))))
...
(ert-deftest mule-enter-dwim-non-editing-enter-handler-info-mode-valid-ret ()
  ...
  (let ((expected-cmd #'Info-follow-nearest-node))
    (mule-enter-dwim-test--with-local-ret expected-cmd 'Info-mode
                                          (let ((result (mule--non-editing-enter-handler)))
                                            (should (eq result expected-cmd))))))
...
fboundp, returns it anyway.  Caller is responsible for invocation.
Expected: non-nil."
  (mule-enter-dwim-test--with-local-ret 'nonexistent-command-xyz 'dired-mode
                                        (should (mule--non-editing-enter-handler))))
...
(ert-deftest mule-enter-dwim-non-editing-enter-handler-ret-bound-to-lambda ()
  "In a non-editing mode where RET is bound to a lambda, returns it.
  The handler returns any non-nil, non-undefined, non-keymap binding.
  Expected: non-nil (the lambda object)."
