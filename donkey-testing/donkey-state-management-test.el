;;; donkey-state-management-test.el --- Tests for DONKEY state management -*- lexical-binding: t; -*-
...
;; ===========================================================================
;; Section: donkey-indicator
;; Selector: (ert "donkey-state-indicator")
;; ===========================================================================
...
;; ===========================================================================
;; Section: donkey--exit-insert
;; Selector: (ert "donkey-state-exit-insert")
;; ===========================================================================
...
(ert-deftest donkey-state-exit-insert-calls-donkey-enter-normal ()
  "Calls donkey-enter-normal to switch to normal mode.
            Expected: donkey-enter-normal invoked."
  (let (called)
    (with-temp-buffer
      (let ((donkey-normal-mode t))
        (cl-letf (((symbol-function 'minibufferp)
...
(ert-deftest donkey-state-exit-insert-force-enables-normal-if-still-off ()
  "If donkey-enter-normal doesn't activate normal mode, force-enables it
            ...
            Expected: donkey-normal-mode called with arg 1."
  (let (force-arg)
    (with-temp-buffer
      (let ((donkey-normal-mode nil))
        (cl-letf (((symbol-function 'minibufferp)
...
(ert-deftest donkey-state-exit-insert-skips-force-when-normal-active ()
  "When donkey-enter-normal successfully enables normal mode, the
            ...
            Expected: donkey-normal-mode not called directly."
  (let (force-called)
    (with-temp-buffer
      (let ((donkey-normal-mode t))
        (cl-letf (((symbol-function 'minibufferp)
...
(ert-deftest donkey-state-exit-insert-minibuffer-delegates-to-keyboard-quit ()
  "In the minibuffer, delegates to keyboard-quit and skips all other steps.
            Expected: keyboard-quit called, deactivate-mark and donkey-enter-normal skipped."
...
;; ===========================================================================
;; Section: donkey--intercept-quit-in-insert
;; Selector: (ert "donkey-state-intercept-quit")
;; ===========================================================================
...
;; ===========================================================================
;; Section: donkey--on-normal-entry
;; Selector: (ert "donkey-state-on-normal-entry")
;; ===========================================================================

(ert-deftest donkey-state-on-normal-entry-saves-and-deactivates-input-method ()
  "When donkey-normal-mode is active and an input method is active,
            saves the method name and deactivates it.
            Expected: donkey--saved-input-method set, deactivate-input-method called."
  (let (deactivated)
    (with-temp-buffer
      (let ((donkey-normal-mode t)
...
(ert-deftest donkey-state-on-normal-entry-skips-when-no-input-method ()
  "When no input method is active, does nothing.
            Expected: donkey--saved-input-method unchanged, deactivate not called."
  (let (deactivated)
    (with-temp-buffer
      (let ((donkey-normal-mode t)
...
;; ===========================================================================
;; Section: donkey--on-insert-entry
;; Selector: (ert "donkey-state-on-insert-entry")
;; ===========================================================================
...
(ert-deftest donkey-state-on-insert-entry-skips-when-mode-disabled ()
  "When donkey-insert-mode is nil, does nothing.
            Expected: activate-input-method not called."
  (let (activated)
    (with-temp-buffer
      (let ((donkey-insert-mode nil)
...
;; ===========================================================================
;; Section: donkey--on-input-method-activate
;; Selector: (ert "donkey-state-on-input-method-activate")
;; ===========================================================================
...
(ert-deftest donkey-state-on-input-method-activate-skips-when-no-method ()
