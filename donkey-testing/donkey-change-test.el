;;; donkey-change-test.el --- Tests for donkey-change -*- lexical-binding: t; -*-
...
;; ===========================================================================
;; Section: donkey-change
;; Selector: (ert "donkey-change")
...
(ert-deftest donkey-change-no-region-enters-insert ()
  "Without region, enters insert mode after deletion.
    Expected: donkey-enter-insert called exactly once."
  (let ((call-count 0))
    (with-temp-buffer
      (insert "hello\n")
      (goto-char 1)
...
(ert-deftest donkey-change-no-region-last-char ()
...
(ert-deftest donkey-change-region-deletes-region ()
  "With an active region (not rectangle), deletes from mark to point.
    ...
    Expected: delete-region called with (1, 6), donkey-enter-insert called."
...
(ert-deftest donkey-change-region-enters-insert ()
...
(ert-deftest donkey-change-region-mark-before-point ()
...
(ert-deftest donkey-change-region-single-char-region ()
...
(ert-deftest donkey-change-rectangle-mode-enters-insert ()
  "In rectangle mode, still enters insert mode after string-rectangle.
    Expected: donkey-enter-insert called."
  (let (entered)
    (with-temp-buffer
      (insert "hello\n")
      (goto-char 1)
...
(ert-deftest donkey-change-call-interactively-no-region ()
  "Can be called interactively without a region.
    Expected: no error, delete-char and donkey-enter-insert execute."
  (let (entered)
    (with-temp-buffer
      (insert "hello\n")
      (goto-char 1)
...
(ert-deftest donkey-change-call-interactively-with-region ()
  "Can be called interactively with a region.
    Expected: no error, delete-region and donkey-enter-insert execute."
  (let (entered)
    (with-temp-buffer
      (insert "hello world\n")
      (goto-char 6)
    ...
    ;;; donkey-change-test.el ends here
